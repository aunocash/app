...
import { FiArrowRight, FiCheckCircle, FiCode, FiExternalLink, FiLock, FiShield } from "react-icons/fi";
  ...
  return <>
    <header className="docs-portal-header">
      ...
      <nav className="docs-portal-links" aria-label="Documentation links">
        <a href="#getting-started">Guides</a><a href="/developers">API reference</a><a href="#api-status">SDK status</a><a href="/roadmap">Roadmap</a><a className="docs-portal-launch" href="/dashboard/create">Launch app <FiExternalLink aria-hidden="true" /></a>
      </nav>
    </header>
    <main className="docs-portal">
      <div className="docs-portal-mobile-nav"><span>On this page</span><a href="#overview">Overview</a><a href="#checkout-flow">Checkout flow</a><a href="#api-status">API status</a><a href="#roadmap">Roadmap</a></div>
      ...
      <article className="docs-portal-content">
        ...
        <section className="docs-portal-hero" id="overview">
          ...
          <p className="docs-portal-lead">Implementation-aligned documentation for payment links, wallet-native checkout, and verified settlement on Solana devnet.</p>
          <div className="docs-portal-hero-actions"><a className="button" href="#getting-started">Start here <FiArrowRight aria-hidden="true" /></a><a className="docs-portal-text-link" href="/developers">View API surface <FiArrowRight aria-hidden="true" /></a></div>
        ...
        <Section id="getting-started" eyebrow="01 / GET STARTED" title="Understand the payment path.">
          <p>AUNO is a non-custodial payment application for Solana devnet. A merchant signs an immutable payment intent, a payer reviews the stored request, and the server verifies the finalized transaction before a receipt is created.</p>
          <div className="docs-portal-callout"><strong>Developer preview</strong><span>Real wallet acceptance tests for SOL and USDC are still outstanding. Do not treat this release as an audited production payment processor.</span></div>
          ...
          <p>Use the <a href="https://faucet.solana.com" target="_blank" rel="noreferrer">Solana faucet</a> for test SOL. Never fund devnet testing with mainnet assets.</p>
        </Section>
        <Section id="payment-links" eyebrow="02 / PAYMENT LINKS" title="One link, one persisted request.">
        ...
        <Section id="checkout-flow" eyebrow="03 / WALLET-NATIVE CHECKOUT" title="Review first. Sign once.">
          <p>Checkout loads the stored payment intent rather than trusting query parameters. The server prepares a transaction from that intent. The wallet signs it, and the server checks that the signed message exactly matches the prepared transaction before submission.</p>
        ...
        <Section id="verification" eyebrow="04 / TRANSACTION VERIFICATION" title="A signature is not a receipt.">
          <p>PAID is written only by server verification against finalized Solana RPC data. Verification checks successful execution, payer signature, the payment memo, the prepared message, asset, amount, and destination.</p>
        ...
        <Section id="payment-status" eyebrow="05 / PAYMENT LIFECYCLE" title="Status is server-authoritative.">
        ...
        <Section id="receipts" eyebrow="06 / VERIFIED RECEIPTS" title="Show the facts that settled.">
          <p>A verified receipt includes the payment ID, amount, asset, recipient, payer, network, transaction signature, and chain timestamp. Explorer links explicitly use <code>cluster=devnet</code>.</p>
        </Section>
        <Section id="api-status" eyebrow="07 / DEVELOPER INFRASTRUCTURE" title="The public API is not launched yet.">
        ...
        <Section id="security-model" eyebrow="08 / SECURITY MODEL" title="Keep custody with the wallet.">
        ...
        <Section id="network" eyebrow="09 / NETWORK CONFIGURATION" title="Devnet is intentional.">
          <p><code>SOLANA_NETWORK</code> must be <code>devnet</code>. Mainnet is rejected. SOL uses nine decimal places; the configured Circle devnet USDC mint uses six. Public RPC endpoints may throttle, so a dedicated devnet RPC is recommended for reliable operation.</p>
        </Section>
        <Section id="roadmap" eyebrow="10 / ROADMAP" title="From payment foundations to programmable money.">
          <p>Split payments are available as a deterministic allocation preview but live multi-recipient settlement is not enabled. Escrow, milestones, subscriptions, webhooks, the public API, and SDK remain planned or under research.</p>
