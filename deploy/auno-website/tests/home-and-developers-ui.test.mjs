...
assert.match(paymentUi, /const EmbeddedCheckout = lazy/);
assert.match(paymentUi, /checkoutVisited/);
assert.match(paymentUi, /hidden=\{!showCheckout\}/);
assert.match(paymentUi, /initialPayment=\{created\}/);
...
assert.doesNotMatch(mainnetSite, /allowlisted|approved merchant|ALLOWLIST/i);
...
assert.match(paymentUi, /MainnetBetaRibbon features="USDC and split payments"/);
assert.match(mainnetSite, /MainnetBetaRibbon features="USDC and split payments"/);
