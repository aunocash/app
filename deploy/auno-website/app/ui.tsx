...
export function Footer({ network }: { network?: SiteNetwork }) {
  ...
  return <footer><div><Brand /><p>Programmable Payments on Solana.</p><div className="contract-address"><span>CA</span><code title="Swipe horizontally to view the full AUNO contract address" aria-label="AUNO contract address" tabIndex={0}>{CONTRACT_ADDRESS}</code><button type="button" onClick={copyContractAddress} aria-label="Copy AUNO contract address"><FiCopy aria-hidden="true" /></button></div></div><div><a href="/#product">Product</a><a href="/developers">Developers</a><a href="/docs">Documentation</a><a href="/whitepaper">Whitepaper</a><a href="/roadmap">Roadmap</a><a className="social-link footer-social-link" href="https://x.com/aunocash" target="_blank" rel="noreferrer" aria-label="Follow AUNO on X"><FaXTwitter aria-hidden="true" /></a></div><div className="footer-bottom"><span>© 2026 AUNO</span><span>Value in motion.</span><span>{mainnet ? "Mainnet Beta" : "Designed for auno.cash"}</span></div></footer>;
}

export function MainnetBetaRibbon({ features }: { features: string }) {
  function showAvailability() {
    toast.info("Not available on Mainnet Beta.", {
      description: `${features} are not released in the current Mainnet Beta scope.`,
    });
  }

  return <button type="button" className="devnet-only-ribbon" onClick={showAvailability} aria-label={`${features} are unavailable on Mainnet Beta. Show details.`}><FiLock aria-hidden="true" /><span>MAINNET BETA</span><strong>{features} unavailable</strong></button>;
}
...
export function PaymentCard({ network }: { network?: SiteNetwork }) {
  const mainnet = useMainnetSite(network);
  const asset = mainnet ? "SOL" : "USDC";
  const amount = mainnet ? "0.1" : "100";
  return <div className="payment-scene"><span className="scene-label">A PAYMENT. A WORLD OF POSSIBILITIES.</span><div className="float-tag"><FiArrowUpRight className="inline-icon" aria-hidden="true" /> Made to move with you</div><div className="payment-card"><div className="card-top"><span className="mini-brand"><img src="/auno-logo.png" alt="" width="32" height="32" /> AUNO</span><span className="badge">{mainnet ? "MAINNET BETA" : "EXAMPLE"}</span></div><div className="invoice-icon"><FiSend aria-hidden="true" /></div><h3>Website Development</h3><p className="card-sub">Payment request</p><div className="amount">{amount}<span>{asset}</span></div><div className="card-detail"><span>Recipient</span><span>8Ks…91Q <FiArrowUpRight className="inline-icon" aria-hidden="true" /></span></div><div className="card-detail"><span>Network</span><span>{mainnet ? "Solana Mainnet Beta" : "Solana Devnet"}</span></div><a className="button wide" href="/dashboard/create">Create a payment like this <LaunchIcon /></a><p className="card-foot"><FiShield className="inline-icon" aria-hidden="true" /> Non-custodial by design</p></div><div className="settle-tag"><FiCornerDownRight aria-hidden="true" /><div>One link. Direct settlement.<small>Illustrative preview · No funds moved</small></div></div></div>;
}

export function Home({ network }: { network?: SiteNetwork }) {
  const mainnet = useMainnetSite(network);
  const mainnetSplits = useMainnetSplitsCapability(mainnet);
  const features = mainnet ? [
    { ...productFeatures[0], status: "MAINNET BETA", description: "Create a bounded SOL payment link with a fixed amount and recipient." },
    { ...productFeatures[1], status: "MAINNET BETA", description: "Wallet-managed signing with finalized server verification." },
    { ...productFeatures[2], status: mainnetSplits ? "MAINNET BETA" : "UNAVAILABLE", description: mainnetSplits ? "Route a SOL payment to multiple recipients in one verified transaction." : "Available only when the Mainnet split feature policy is enabled." },
    ...productFeatures.slice(3),
  ] : productFeatures;
  return <><Nav network={network} /><main><section className="hero"><Flow /><div className="hero-copy"><div className="eyebrow"><span className="line" /> PAYMENT INFRASTRUCTURE FOR SOLANA</div><h1>Money should be<br /><em>programmable.</em></h1><p className="lead">{mainnet ? <>Create bounded SOL payment links,<br className="desktop" /> with finalized receipts on Solana.</> : <>Create payment links, accept SOL and USDC,<br className="desktop" /> and build programmable payment flows on Solana.</>}</p><div className="actions"><a className="button" href="/dashboard/create">{mainnet ? "Open Mainnet Beta" : "Try on Devnet"} <LaunchIcon /></a><a className="text-link" href="/developers">Explore Developers <ActionArrow /></a></div><p className="hero-status"><strong>{mainnet ? "Mainnet Beta · Solana Mainnet" : "Public Beta · Solana Devnet"}</strong><span>{mainnet ? "Bounded payment links with wallet-managed checkout and finalized receipts." : "Test payment flows with SOL and USDC before mainnet release."}</span></p><div className="hero-trust"><span><SiSolana aria-hidden="true" /> Built on Solana</span></div></div><PaymentCard network={network} /><div className="hero-bottom"><span>VALUE IN MOTION</span><span>Built for the way money should move.</span><a href="#product" aria-label="Explore product"><FiArrowDown aria-hidden="true" /></a></div></section><div className="network-strip"><span>A simpler payment layer.</span><span>Payment links</span><span>Wallet-native checkout</span><span>Verified receipts</span></div><section id="product" className="section feature-two"><div><div className="eyebrow">01 / PAYMENT LINKS</div><h2>One link.<br />Ready to get paid.</h2><p>Create a payment request with a fixed amount and recipient. Share the link, and let your customer pay directly from their wallet.</p><a className="text-link" href="/dashboard/create">Create your first link <ActionArrow /></a><div className="detail-note">{mainnet ? <>SOL · Mainnet Beta<br />Payment links follow the active Mainnet policy and cap.</> : <>SOL + USDC · Devnet only<br />Payment flows await real wallet acceptance testing.</>}</div></div><div className="link-preview"><div className="panel-title">Create Payment <span className="badge">{mainnet ? "MAINNET BETA" : "DEVNET"}</span></div><label>Title<div className="mock-input">Website Development</div></label><div className="two"><label>Amount<div className="mock-input">100.00</div></label><label>Asset<div className="mock-input mock-select">USDC <FiChevronDown className="inline-icon" aria-hidden="true" /></div></label></div><label>Recipient<div className="mock-input muted">Your Solana wallet address</div></label><a className="button wide" href="/dashboard/create">Open Payment Creator <ActionArrow /></a></div></section><section className="section checkout-band"><div className="eyebrow">02 / WALLET-NATIVE CHECKOUT</div><h2>Checkout without the friction.</h2><p>The amount, asset, network, and destination stay visible.<br />You approve the transfer in your own wallet.</p><div className="steps">{[["01", "CONNECT", "Choose your Solana wallet."], ["02", "REVIEW", "Confirm the payment details."], ["03", "SIGN", mainnet ? "Authorize on mainnet." : "Authorize on devnet."], ["04", "VERIFY", "Receive a verified receipt."]].map((step) => <div key={step[0]}><span>{step[0]}</span><h4>{step[1]}</h4><p>{step[2]}</p></div>)}</div><a className="text-link" href="/docs#checkout-flow">See how checkout works <ActionArrow /></a></section><section id="split-payments" className="section feature-two split-feature"><div><div className="eyebrow">03 / SPLIT PAYMENTS <span className="badge">{mainnet ? (mainnetSplits ? "MAINNET BETA" : "UNAVAILABLE") : "PLANNED"}</span></div><h2>One payment.<br />Multiple destinations.</h2><p>{mainnet ? "Route one SOL payment to multiple destinations only when the active Mainnet split policy permits it." : "Define where value goes before the payment happens. A single transaction can route a payment to your team, partners, and treasury."}</p><a className="text-link" href="/split">{mainnet && !mainnetSplits ? "View Split availability" : "Explore Split Payment"} <ActionArrow /></a>{mainnet && !mainnetSplits ? <MainnetBetaRibbon features="Split payments" /> : <p className="detail-note">{mainnet ? "One transaction, every recipient, finalized verification." : "Interactive allocation preview. Live split settlement is not enabled."}</p>}</div><SplitFlow /></section><section className="section"><div className="section-heading"><div><div className="eyebrow">THE PAYMENT TOOLKIT</div><h2>Payments that follow your rules.</h2></div><a href="/roadmap" className="text-link">View roadmap <ActionArrow /></a></div><div className="feature-grid">{features.map((feature) => { const Icon = feature.icon; return <a className="feature" href={feature.href} key={feature.title}><div className="feature-top"><span className="feature-icon"><Icon aria-hidden="true" /></span><span className="badge">{feature.status}</span></div><h3>{feature.title}</h3><p>{feature.description}</p></a>; })}</div></section><section className="developer-section section feature-two"><div><div className="eyebrow">FOR THE BUILDERS</div><h2>Built for developers.</h2><p>Add Solana payments to your product, with transparent payment states and independently verified settlement.</p><div className="actions"><a className="button light" href="/docs">Read the Docs <LaunchIcon /></a><a className="text-link" href="/developers">Explore API <ActionArrow /></a></div></div><div className="code-panel"><div><span>payments.ts</span><span className="badge">PROPOSED SDK</span></div><pre><code>{'const payment = await auno.payments.create({\n  title: "Website Development",\n  amount: "100",\n  asset: "USDC",\n  recipient: merchantWallet,\n});\n\nreturn payment.checkoutUrl;'}</code></pre><small>Design preview. @auno/sdk is not published.</small></div></section><section className="section ecosystem"><div><div className="eyebrow">THE AUNO ECOSYSTEM</div><h2>Infrastructure comes first.</h2><p>$AUNO is the proposed ecosystem token. SOL and USDC payments do not require it. No token contract, launch, or utility is verified in this release.</p><a href="/whitepaper" className="text-link">Read the whitepaper <ActionArrow /></a></div><div className="token-label">$AUNO<span className="badge">COMING SOON · PROPOSED</span></div></section><section className="final-cta"><div className="eyebrow">YOUR NEXT PAYMENT STARTS HERE</div><h2>Move value your way.</h2><p>Create programmable payment flows on Solana.</p><div className="actions"><a className="button" href="/dashboard/create">Launch AUNO <LaunchIcon /></a><a className="text-link" href="/docs">Read the Docs <ActionArrow /></a></div></section></main><Footer network={network} /></>;
}
