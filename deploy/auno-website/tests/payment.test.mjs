...
const sqlite=new DatabaseSync(':memory:');sqlite.exec(readFileSync('drizzle/0000_lush_the_executioner.sql','utf8'));
...
Connection.prototype.getGenesisHash=async()=> 'EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG';Connection.prototype.getLatestBlockhash=async()=>({blockhash:Keypair.generate().publicKey.toBase58(),lastValidBlockHeight:10000});Connection.prototype.getBlockHeight=async()=>100;Connection.prototype.sendRawTransaction=async bytes=>bs58.encode(Transaction.from(bytes).signature);
const prepared=await server.preparePayment(request({payer:payer.publicKey.toBase58()}),p.id);const transaction=Transaction.from(Buffer.from(prepared.transaction,'base64'));transaction.sign(payer);
const submitted=await server.submitPayment(request({attemptId:prepared.attemptId,transaction:transaction.serialize().toString('base64')}),p.id);assert.equal(submitted.status,'SUBMITTED');passed++;console.log('PASS exact signed submission');
const memo=`auno:${p.id}:${prepared.attemptId}`;
let wrong=false,failed=false;
Connection.prototype.getParsedTransaction=async()=>({blockTime:Math.floor(Date.now()/1000),meta:{err:failed?{InstructionError:[0,'failed']}:null},transaction:{message:{accountKeys:[{pubkey:payer.publicKey,signer:true},{pubkey:merchant.publicKey,signer:false}],instructions:[{programId:SystemProgram.programId,parsed:{type:'transfer',info:{source:payer.publicKey.toBase58(),destination:wrong?Keypair.generate().publicKey.toBase58():merchant.publicKey.toBase58(),lamports:1000000}}},{programId:new PublicKey(model.MEMO_PROGRAM),parsed:memo}]}}});Connection.prototype.getTransaction=async()=>({transaction:{message:transaction.compileMessage()}});
wrong=true;await assert.rejects(()=>server.verifyPayment(p.id,submitted.signature));assert.notEqual((await server.getPayment(p.id)).status,'PAID');wrong=false;failed=true;await assert.rejects(()=>server.verifyPayment(p.id,submitted.signature));failed=false;passed++;console.log('PASS wrong recipient and failed transaction cannot become PAID');
const paid=await server.verifyPayment(p.id,submitted.signature);assert.equal(paid.status,'PAID');assert.equal((await server.verifyPayment(p.id,submitted.signature)).transactionSignature,submitted.signature);await assert.rejects(()=>server.verifyPayment(p.id,bs58.encode(new Uint8Array(64).fill(2))));passed++;console.log('PASS verified receipt, duplicate verification and wrong signature protection');
const missingPayload=payload.replace('TEST','MISSING');const missingSig=bs58.encode(nacl.sign.detached(new TextEncoder().encode(model.creationMessage(missingPayload)),merchant.secretKey));const missing=await server.createPayment(request({payload:missingPayload,signature:missingSig}));const missingPrepared=await server.preparePayment(request({payer:payer.publicKey.toBase58()}),missing.id);const missingTransaction=Transaction.from(Buffer.from(missingPrepared.transaction,'base64'));missingTransaction.sign(payer);const missingSubmitted=await server.submitPayment(request({attemptId:missingPrepared.attemptId,transaction:missingTransaction.serialize().toString('base64')}),missing.id);Connection.prototype.getParsedTransaction=async()=>null;Connection.prototype.getSignatureStatus=async()=>({value:null});assert.equal((await server.verifyPayment(missing.id,missingSubmitted.signature)).status,'FAILED');passed++;console.log('PASS missing signature is failed instead of confirming');
